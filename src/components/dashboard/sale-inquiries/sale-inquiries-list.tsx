...
export function SaleInquiriesList({
  inquiries,
}: {
  ...
  if (inquiries.length === 0) {
    return (
      <p className="rounded-lg border border-dashed p-6 text-sm text-muted-foreground">
        No buyer inquiries yet. When someone scans your dash-card sale QR and
        submits the contact form, inquiries appear here.
      </p>
    );
  }
  ...
  return (
    <ul className="divide-y rounded-lg border">
      {inquiries.map((inquiry) => (
        <li key={inquiry.id}>
          <Link
            href={`/dashboard/sale-inquiries/${inquiry.id}`}
          ...
          >
            <div className="min-w-0 space-y-1">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {saleInquiryNeedsAttention(inquiry.status) ? (
              ...
              <p className="text-sm text-muted-foreground">{inquiry.eventLabel}</p>
            </div>
